...
const InvoiceViewer = ({ open, userId, userName, onClose }) => {
    ...
    const [invoiceInfo,    setInvoiceInfo]    = useState(null);   // { file_name, mime_type, url (blob) }
    const [error,          setError]          = useState(null);
    const [fetchAttempted, setFetchAttempted] = useState(false);

    // Fetch invoice blob and create a local object URL for preview
    const fetchInvoice = useCallback(async () => {
        ...
        try {
            ...
            setInvoiceInfo({
                file_name: userName ? `Invoice — ${userName}` : `invoice_${userId}`,
    ...
    // ── Open in new tab ──────────────────────────────────────────────────────
    // If the blob URL is already available from the preview fetch, use it directly
    // (synchronous call → no popup blocking risk).
    // Otherwise open the tab first and navigate after the fetch resolves.
    const handleView = useCallback(() => {
        if (invoiceInfo?.url) {
            // ✅ Blob URL already ready — open directly (synchronous, no popup risk)
            ...
            <DialogContent sx={{ p: 3, bgcolor: T.bg }}>
                {loading && !invoiceInfo ? (
                    ...
                    <Box sx={{
                        ...
                        <Box component="button" type="button" onClick={fetchInvoice}
                             sx={{
                        ...
                        <Box sx={{
                            ...
                            <Grid container spacing={2.5}>
                                <Grid item xs={12}>
                                    <InfoItem
                                        label="File Type"
                                        value={invoiceInfo.mime_type?.split('/')[1]?.toUpperCase() || 'Unknown'}
                                    />
                                </Grid>
                            </Grid>
                        </Box>
