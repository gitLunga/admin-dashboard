...
const StatusUpdateModal = ({ open, user, onClose, onSubmit, submitting = false }) => {
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth
            ...
            <DialogContent sx={{ p: 3, bgcolor: T.bg }}>
                ...
                {user && (
                    <Box sx={{ p: 2, mb: 3, borderRadius: '12px', bgcolor: T.surface, border: `1px solid ${T.border}` }}>
                        <Typography sx={{ fontSize: '0.68rem', fontWeight: 700, color: T.muted, textTransform: 'uppercase', letterSpacing: 0.8, mb: 0.5 }}>User</Typography>
                        ...
                        {user.registration_status === 'Verified' && (
                            <Box sx={{ mt: 1, px: 1.5, py: 0.8, borderRadius: '8px', bgcolor: T.greenSoft, border: `1px solid ${T.green}22` }}>
                                <Typography sx={{ fontSize: '0.75rem', color: T.green, fontWeight: 600 }}>
                                    ⚠️ This user is already Verified. Status changes are locked.
                                </Typography>
                            </Box>
                        )}
                    </Box>
                ...
                {error && (
                    <Box sx={{ mb: 2.5, px: 2, py: 1.5, borderRadius: '10px', bgcolor: T.roseSoft, border: `1px solid ${T.rose}28` }}>
                        <Typography sx={{ fontSize: '0.82rem', color: T.rose, fontWeight: 600 }}>{error}</Typography>
                    </Box>
                )}

                {/* Status selection cards */}
                <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: T.muted, textTransform: 'uppercase', letterSpacing: 0.8, mb: 1.2 }}>
                    Select Status
                </Typography>
                <Box sx={{ display: 'flex', gap: 1.2, mb: 2.5, flexWrap: 'wrap' }}>
                    {STATUS_OPTIONS.map(opt => {
                        const selected = status === opt.value;
                        const isLocked = user?.registration_status === 'Verified' && opt.value !== 'Verified';
                        return (
                            <Box key={opt.value}
                                 onClick={() => { if (!isLocked) { setStatus(opt.value); setError(''); } }}
                                 sx={{
                                     flex: 1, minWidth: 100, p: 1.6, borderRadius: '12px',
                                     cursor: isLocked ? 'not-allowed' : 'pointer',
                                     opacity: isLocked ? 0.4 : 1,
                                     border: `2px solid ${selected ? opt.color : T.border}`,
                                     bgcolor: selected ? opt.soft : T.surface,
                                     transition: 'all 0.16s ease',
                                     '&:hover': isLocked ? {} : { borderColor: opt.color, bgcolor: opt.soft },
                                 }}>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, mb: 0.4 }}>
                                    <opt.Icon sx={{ fontSize: 16, color: selected ? opt.color : T.muted }} />
                                    <Typography sx={{ fontWeight: 700, fontSize: '0.84rem', color: selected ? opt.color : T.text }}>
                                        {opt.label}
                                    </Typography>
                                </Box>
                                <Typography sx={{ fontSize: '0.69rem', color: T.muted, lineHeight: 1.4 }}>{opt.desc}</Typography>
                            </Box>
                        );
                    })}
                </Box>
                ...
                {/* Notes field */}
                <Box>
                    <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: T.muted, textTransform: 'uppercase', letterSpacing: 0.8, mb: 1 }}>
                        Verification Notes
                        {status === 'Rejected'
                    ...
                    <TextField fullWidth multiline rows={4}
                               placeholder={status === 'Rejected' ? 'Provide a clear reason for rejection…' : 'Add notes about this verification…'}
                               value={notes} onChange={e => setNotes(e.target.value)} required={status === 'Rejected'}
                               sx={{
            ...
            <DialogActions sx={{ px: 3, py: 2.2, bgcolor: T.surface, borderTop: `1px solid ${T.border}`, gap: 1.5 }}>
                ...
                <Button onClick={handleSubmit} variant="contained" disabled={submitting || user?.registration_status === 'Verified'}
                        sx={{
                            ...
                            '&:hover': { bgcolor: status === 'Verified' ? '#047857' : status === 'Rejected' ? '#B91C1C' : '#B45309', boxShadow: `0 4px 14px ${selectedMeta.color}44` },
                            '&.Mui-disabled': { bgcolor: T.border, color: T.muted },
                        }}>
                    {submitting ? 'Updating…' : status === 'Verified' ? 'Verify User' : status === 'Rejected' ? 'Reject User' : 'Set as Pending'}
                </Button>
            </DialogActions>
        </Dialog>
    );
