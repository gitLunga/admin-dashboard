...
const AdminApplications = () => {
    ...
    const [allApplications, setAllApplications] = useState([]);  // raw from server
    const [applications, setApplications] = useState([]);  // filtered display
    ...
    // Client-side filtering — runs instantly on every filter/search change
    useEffect(() => {
    ...
    const handleClearFilters = () => {
        setFilters({status: 'all', user_type: 'all', region: 'all'});
        setSearchTerm('');
    };
    ...
    const handleStatusUpdate = async (appId, statusData) => {
        ...
        if (app && ['Approved', 'Rejected', 'Cancelled'].includes(app.application_status)) {
        ...
        try {
            ...
            if (response.data?.success) {
                const msg = response.data.message || `Application #${appId} status updated to ${statusData.status}.`;
                success(msg, 'Application Updated');
            ...
            if (status === 409) {
                warning(msg, 'Status Locked');
            } else if (status === 422) {
                toastError(msg, 'Not Eligible');
            } else {
                                        ...
                                        <TableRow key={app.application_id} hover sx={{
                                                ...
                                                <Box sx={{display: 'flex', alignItems: 'center', gap: 1.2}}>
                                                    ...
                                                    <Box>
                                                        ...
                                                        <Box
                                                            ...
                                                            {app.user_type && <Chip label={app.user_type} size="small"
                                                                                    sx={{
                                                                                        height: 18,
                                                                                        fontSize: '0.65rem',
                                                                                        fontWeight: 600,
                                            ...
                                            {!isMobile && (
                                                <TableCell sx={{py: 1.8, borderBottom: `1px solid ${T.border}`}}>
                                                    ...
                                                    <Box
                                                        sx={{display: 'flex', gap: 0.5, mt: 0.4, alignItems: 'center'}}>
                                            ...
                                            <TableCell sx={{py: 1.8, borderBottom: `1px solid ${T.border}`}}>
                                                <Box sx={{display: 'flex', gap: 0.5}}>
                                                    ...
                                                    <Tooltip
                                                        title={isLocked ? `Status is ${app.application_status} — cannot change` : 'Update Status'}>
                                                        <span>
                                                            <IconButton size="small"
                                                                        ...
                                                                        sx={{
                                                                            ...
                                                                            height: 28,
                                                                            borderRadius: '8px',
                                                                            bgcolor: isLocked ? T.bg : T.amberSoft,
                                                                            color: isLocked ? T.muted : T.amber,
                                                                            '&:hover': {bgcolor: isLocked ? T.bg : '#FDE68A'}
                                                                        }}>
