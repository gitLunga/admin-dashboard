...
/* ── Date formatter — unchanged logic ── */
...
const RecentRegistrations = ({ data = [] }) => {
    ...
    const handleViewUser = (user) => {
        if (user.user_type === 'client') { navigate(`/client-users/${user.id}`); }
        else                             { navigate(`/operational-users/${user.id}`); }
    };
    ...
    if (data.length === 0) {
        return (
            <Box sx={{ textAlign: 'center', py: 6 }}>
                <Box sx={{ width: 52, height: 52, borderRadius: '14px', bgcolor: T.bg, border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 1.5 }}>
                    <EmptyIcon sx={{ fontSize: 24, color: T.muted }} />
                </Box>
                <Typography sx={{ fontWeight: 700, fontSize: '0.92rem', color: T.text, mb: 0.5, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
    ...
    /* ── Table header ── */
    const COL = { user: '30%', type: '11%', status: '13%', contact: '26%', date: '15%', actions: '5%' };
        ...
        <Box>
            ...
            {data.slice(0, visibleCount).map((user) => (
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, minWidth: 0 }}>
                        ...
                        <Box sx={{ minWidth: 0 }}>
                            <Typography sx={{
                                fontWeight: 700, fontSize: '0.83rem', color: T.text,
                                fontFamily: 'Plus Jakarta Sans, sans-serif',
                            ...
                            <Typography sx={{
                                fontSize: '0.71rem', color: T.muted,
                                fontFamily: 'Plus Jakarta Sans, sans-serif',
                                whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                            }}>
                                {user.email}
                    ...
                    {/* Type */}
                    <Box><TypeChip type={user.user_type} /></Box>
                    ...
                    <Box>
                        ...
                        {user.phone_number && (
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.3 }}>
                                ...
                                <Typography sx={{ fontSize: '0.71rem', color: T.muted, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
                                    {user.phone_number}
                                </Typography>
                            </Box>
                        )}
            ...
            {data.length > visibleCount && (
                <Box sx={{ textAlign: 'center', mt: 1.5 }}>
                    <Box
                        component="button" type="button" onClick={loadMore}
                        sx={{
                            border: `1.5px solid ${T.border}`, borderRadius: '9px',
                            px: 2, py: 0.8, bgcolor: T.bg, cursor: 'pointer',
                            fontFamily: 'Plus Jakarta Sans, sans-serif',
                            fontWeight: 600, fontSize: '0.78rem', color: T.muted,
                            '&:hover': { bgcolor: T.accentSoft, color: T.accent, borderColor: T.accent },
                            transition: 'all 0.15s',
                        }}
                    >
