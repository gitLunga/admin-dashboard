...
const ChartTooltip = ({active, payload, label}) => {
    ...
    return (
        <Box sx={{
            ...
            {label &&
                <Typography sx={{fontSize: '0.75rem', fontWeight: 700, color: T.text, mb: 0.8}}>{label}</Typography>}
            {payload.map((p, i) => (
                <Box key={i} sx={{display: 'flex', alignItems: 'center', gap: 1, mb: 0.3}}>
                    ...
                    <Typography className="mono" sx={{
                        ...
                        fontWeight: 700,
                        color: T.text
                    }}>{p.value?.toLocaleString()}</Typography>
                </Box>
            ))}
        </Box>
    );
...
const Statistics = () => {
    ...
    const operationalRoleData = basicStats?.operational_users?.stats?.map(s => ({
        name: s.user_role,
        value: s.count
    })) || [];
        ...
        <Box sx={{p: 3, bgcolor: T.bg}}>
            <Alert severity="error" sx={{borderRadius: '10px'}} action={
                <Button color="inherit" size="small" onClick={fetchAllStatistics}
                        sx={{fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700}}>Retry</Button>
            }>{error}</Alert>
        </Box>
    );
    ...
    return (
        <Box sx={{p: {xs: 2, md: 3.5}, bgcolor: T.bg, minHeight: '100vh'}}>
            ...
            <Box sx={{
                ...
                <Box sx={{display: 'flex', gap: 1.5, flexWrap: 'wrap', alignItems: 'center'}}>
                    <FormControl size="small">
                        <Select value={timeRange} displayEmpty onChange={e => setTimeRange(e.target.value)}
                                renderValue={v => ({
                                    week: 'Last Week',
                                    month: 'Last Month',
                            ...
                            {[['week', 'Last Week'], ['month', 'Last Month'], ['quarter', 'Last Quarter'], ['year', 'Last Year']].map(([v, l]) =>
                                <MenuItem key={v} value={v} sx={{fontSize: '0.83rem'}}>{l}</MenuItem>)}
                        </Select>
                    </FormControl>
                    <Button onClick={handleExportData} startIcon={<DownloadIcon sx={{fontSize: '16px !important'}}/>}
                    ...
                    <Button onClick={fetchAllStatistics} startIcon={<RefreshIcon sx={{fontSize: '16px !important'}}/>}
                            variant="outlined" size="small"
                            sx={{
                                borderRadius: '10px',
                                textTransform: 'none',
            ...
            {dashboardMetrics && (
                <Grid container spacing={2} sx={{mb: 3}}>
                    <Grid item xs={12} sm={4}>
                        <MetricCard icon={TrendingUpIcon} color={dashboardMetrics.daily_growth > 0 ? T.green : T.rose}
                                    soft={dashboardMetrics.daily_growth > 0 ? T.greenSoft : T.roseSoft}
                                    label="Today's Registrations" value={dashboardMetrics.todays_registrations || 0}
                                    sub={`${dashboardMetrics.daily_growth > 0 ? '+' : ''}${dashboardMetrics.daily_growth || 0}% from yesterday`}
                                    delay={0.28}/>
                    </Grid>
                    <Grid item xs={12} sm={4}>
                        <MetricCard icon={TimelineIcon} color={T.accent} soft={T.accentSoft}
                                    label="Avg. Verification Time"
                                    value={`${dashboardMetrics.avg_verification_days || '—'}`}
            ...
            <Paper elevation={0} sx={{
                ...
                {TABS.map((tab, i) => <TabBtn key={tab} label={tab} active={activeTab === i}
                ...
                <Grid container spacing={2.5}>
                    ...
                    <Grid item xs={12} md={6}>
                        <SectionCard title="Client Status Distribution"
                                     subtitle={`Total: ${basicStats?.client_users?.total || 0} clients`}>
                            <Box sx={{height: 280}}>
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie data={clientStatusData} cx="50%" cy="50%" outerRadius={95} innerRadius={45}
                                             dataKey="value" labelLine={false}
                                             label={({name, percent}) => `${name} ${(percent * 100).toFixed(0)}%`}>
                                            {clientStatusData.map((entry, i) => <Cell key={i}
                                                                                      fill={entry.color || CHART_COLORS[i % CHART_COLORS.length]}/>)}
                                        </Pie>
                                        <RechartsTooltip content={<ChartTooltip/>}/>
                                        <Legend iconType="circle" iconSize={8} formatter={v => <span style={{
                                            fontSize: '0.78rem',
                                            color: T.text,
                                            fontFamily: 'Plus Jakarta Sans, sans-serif'
                                        }}>{v}</span>}/>
                    ...
                    {/* Bar chart */}
                    <Grid item xs={12} md={6}>
                        <SectionCard title="Operational User Roles"
                                     subtitle={`Total: ${basicStats?.operational_users?.total || 0} operational users`}>
                            <Box sx={{height: 280}}>
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={operationalRoleData} barSize={32}>
                                        ...
                                        <Bar dataKey="value" name="Users" radius={[6, 6, 0, 0]}>
                                            {operationalRoleData.map((_, i) => <Cell key={i}
                        ...
                        <SectionCard title="Client Status Breakdown">
                            <Box sx={{pt: 1}}>
                                {clientStatusData.map(stat => (
                                    <ProgressRow key={stat.name} name={stat.name} value={stat.value}
                                                 total={basicStats?.client_users?.total || 1} color={stat.color}/>
                                ))}
                                <Box sx={{
                                    pt: 2,
                                    ...
                                    <Typography sx={{fontSize: '0.78rem', fontWeight: 600, color: T.muted}}>Total
                                        Clients</Typography>
                                    <Typography className="mono" sx={{
                                        fontSize: '1.4rem',
                                        fontWeight: 500,
                                        color: T.text
                                    }}>{basicStats?.client_users?.total?.toLocaleString() || 0}</Typography>
                                </Box>
                            </Box>
                        </SectionCard>
                    </Grid>
                    <Grid item xs={12} md={5}>
                        <SectionCard title="Verification Rate">
                            <Box sx={{height: 240}}>
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie data={clientStatusData} cx="50%" cy="50%" outerRadius={85} innerRadius={50}
                                             dataKey="value" paddingAngle={3}>
                                            {clientStatusData.map((entry, i) => <Cell key={i}
                                                                                      fill={entry.color || CHART_COLORS[i % CHART_COLORS.length]}/>)}
                                        </Pie>
                                        <RechartsTooltip content={<ChartTooltip/>}/>
                                        <Legend iconType="circle" iconSize={7} formatter={v => <span style={{
                                            fontSize: '0.75rem',
                                            color: T.text,
                                            fontFamily: 'Plus Jakarta Sans, sans-serif'
                                        }}>{v}</span>}/>
            ...
            {activeTab === 2 && (
                <Grid container spacing={2.5}>
                    {regionData.length > 0 ? (
                        <Grid item xs={12}>
                            <SectionCard title="User Distribution by Region"
                                         subtitle={`${regionData.length} regions with registered users`}>
                                <Box sx={{height: 340}}>
                                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        <Grid item xs={12}>
                            <Paper elevation={0} sx={{
                                ...
                                <Typography sx={{fontSize: '0.88rem', color: T.muted}}>No regional data
            ...
            {activeTab === 3 && monthlyTrends.length > 0 && (
                <Grid container spacing={2.5}>
                    <Grid item xs={12}>
                        <SectionCard title="Registration Trends"
                                     subtitle="Last 6 months · Client &amp; operational registrations">
                            <Box sx={{height: 360}}>
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={monthlyTrends}>
                                        ...
                                        <Area type="monotone" dataKey="clients" stroke={T.accent} strokeWidth={2}
                                              fill="url(#colorClients)" name="Client Registrations"
                                              dot={{r: 3, fill: T.accent}} activeDot={{r: 5}}/>
                                        <Area type="monotone" dataKey="operational" stroke={T.green} strokeWidth={2}
                                              fill="url(#colorOp)" name="Operational Registrations"
                                              dot={{r: 3, fill: T.green}} activeDot={{r: 5}}/>
                                    </AreaChart>
                                </ResponsiveContainer>
                            </Box>
                        </SectionCard>
